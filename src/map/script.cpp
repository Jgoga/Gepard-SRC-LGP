...
BUILDIN_FUNC(getmapxy)
{
	...
	switch (type) {
		case UNITTYPE_MOB:	//Get Mob Position
		   if (script_hasdata(st,6)) {
...
struct map_session_data *sd = NULL;
	...
	sd->state.secure_item = (value) ? 1 : 0;
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(getsecurity)
{
	...
	script_pushint(st,sd->state.secure_item);
	return SCRIPT_CMD_SUCCESS;
...
struct map_session_data *sd = NULL;
	...
	sd->state.restock = (value) ? 1 : 0;
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(getrestock)
{
	struct map_session_data *sd = NULL;
	if (!script_rid2sd(sd))
		return SCRIPT_CMD_SUCCESS;

	script_pushint(st,sd->state.restock);
	return SCRIPT_CMD_SUCCESS;
...
// (^~_~^) Gepard Shield End
	...
	// Restock
	BUILDIN_DEF(setrestock,"i"),
	BUILDIN_DEF(getrestock,""),
